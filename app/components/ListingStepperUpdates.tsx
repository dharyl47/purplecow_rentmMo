...
export default function ListingStepper({ itemData, onSubmit, handleChangeUpdates }: Props) {
	...
	const [listingInfo, updatedListingInfo, ] = useState(itemData);
	

	useEffect(() => {
		updatedListingInfo(itemData);
	}, [itemData]);
	...
	const handleChange = (e: any) => {
		...
		} else updatedListingInfo({ ...listingInfo, [e.target.name]: e.target.value });
	};
		...
		return (
			<>
				<div className={`step-${activeStep}`}>
					{activeStep === 0 && (
						<div>
							<ListingInfoForm handleChange={handleChange} listingInfo={listingInfo} handleChangeUpdates={handleChangeUpdates} />
						</div>
					)}
				</div>
			</>
